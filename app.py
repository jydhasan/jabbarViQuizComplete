...
def quiz_ans():
    ...
    # Extract selected values dynamically from form_data
    selected_values = {}
    for entry in data:
        question_id = str(entry[0])
        selected_values[question_id] = form_data.getlist(question_id)

    # Compare selected values with actual answers in the database
    correct_answers = 0
    incorrect_answers = 0
    for entry in data:
        question_id = str(entry[0])
        actual_answer = entry[2]  # Assuming the correct answer is in the 3rd column
        user_answer = selected_values.get(question_id, [])  # Use an empty list if the question is not answered

        if user_answer and user_answer[0] == actual_answer:
            correct_answers += 1
        else:
            incorrect_answers += 1
######################################################################
 ...
 # Given input string
    ...
    total_count = correct_answers+incorrect_answers
    ...
    try:
        ...
        cursor.execute(f"INSERT INTO {result_table_string} (uname,uphone,schoolname,matchcount,mismatchcount,totalcount,grade) VALUES (?,?,?,?,?,?,?)", (
            current_user.uname, current_user.username, current_user.schoolname, correct_answers, incorrect_answers, total_count, 'A'))
        conn.commit()
        conn.close()
...
################################################################################
    # You can now use the correct_answers and incorrect_answers variables as needed
    result = {
        "correct_answers": correct_answers,
        "incorrect_answers": incorrect_answers
    }

    # For example, convert the result to JSON and send it as the response
